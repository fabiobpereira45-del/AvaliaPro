...
const answers = expandAnswersMatch();
let questionIdx = 0;
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (!line || line.startsWith('Unidade') || line.startsWith('Capítulo') || line.startsWith('PDF') || line.startsWith('GABARITO')) continue;

    if (line.match(/^(Múltipla Escolha|Alternativa INCORRETA):/)) {
        const textPart = line.split(/(a\) |a \))/)[0];
        const choicesPart = line.substring(textPart.length);

        // Parse choices
        const choices = [];
        const aMatch = choicesPart.match(/a\)(.*?)(b\)|c\)|d\)|$)/);
        const bMatch = choicesPart.match(/b\)(.*?)(c\)|d\)|$)/);
        const cMatch = choicesPart.match(/c\)(.*?)(d\)|$)/);
        const dMatch = choicesPart.match(/d\)(.*?)$/);

        const choiceIds = ['a', 'b', 'c', 'd'];
        const choiceTexts = [aMatch, bMatch, cMatch, dMatch].map(m => m ? m[1].trim() : '');

        choiceTexts.forEach((t, j) => {
            if (t) choices.push({ id: `c-${questionIdx}-${choiceIds[j]}`, text: t });
        });

        const ansString = answers[questionIdx].toLowerCase();
        let correctAnswer = '';
        if (ansString.startsWith('a')) correctAnswer = choices[0]?.id;
        if (ansString.startsWith('b')) correctAnswer = choices[1]?.id;
        if (ansString.startsWith('c')) correctAnswer = choices[2]?.id;
        if (ansString.startsWith('d')) correctAnswer = choices[3]?.id;

        parsed.push({
            id: uid(),
            disciplineId: "disc-17",
            type: "multiple-choice",
            text: textPart.replace(/^(Múltipla Escolha|Alternativa INCORRETA):\\s*/, '').trim(),
            choices: choices,
        ...
        questionIdx++;
    } else if (line.match(/^Verdadeiro ou Falso:/)) {
        const textPart = line;
        const ansString = answers[questionIdx].toLowerCase();
        ...
        parsed.push({
            ...
            disciplineId: "disc-17",
            type: "true-false",
            text: textPart.replace(/^Verdadeiro ou Falso:\\s*/, '').replace(/\\(\\s*\\)/, '').trim(),
            choices: [],
            correctAnswer: ansString.startsWith('verdadeiro') || ansString.startsWith('v') ? 'true' : 'false',
            points: 1,
            createdAt: new Date().toISOString()
        });
        questionIdx++;
...
fs.writeFileSync('e:/Projetos AntiGrafity/ESCOLA DE TEOLOGIA/Escola de Teologia/IBAD/public/seed_livros_poeticos.json', JSON.stringify(parsed, null, 2));
