...
const groq = process.env.GROQ_API_KEY ? createOpenAI({
...
LEMBRE-SE: Retorne APENAS o JSON bruto. Sem explicações.`
    ...
    // Implement fallback strategy: Try Groq first (if available), then Gemini Flash as reliable fallback
    const models = []
    ...
    // Always add Gemini as it is configured and reliable
    ...
    for (const model of models) {
      try {
        ...
        console.error("[generate-questions] Model failed, trying next. Error:", err);
        lastError = err;
        continue;
      }
    }
    ...
    if (!generatedTextResult) {
      throw new Error(`Todos os modelos de IA falharam. Último erro: ${lastError?.message || "Erro desconhecido"}`);
    }

    const text = generatedTextResult;
