const fs = require('fs');
const text = `Unidade 1 - O Livro de Jó
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (!line) continue;
    if (line.startsWith('Unidade') || line.startsWith('Capítulo') || line.startsWith('PDF')) continue;

    if (line.includes('Múltipla Escolha:') || line.includes('Alternativa INCORRETA:')) {
        let questionText = line.split(' a)')[0].replace(/^(Múltipla Escolha|Alternativa INCORRETA):\\s*/, '').trim();
        let choicesStr = line.substring(line.indexOf(' a)'));

        let aText = choicesStr.substring(choicesStr.indexOf('a)') + 2, choicesStr.indexOf('b)')).trim();
        let bText = choicesStr.substring(choicesStr.indexOf('b)') + 2, choicesStr.indexOf('c)')).trim();
        let cText = choicesStr.substring(choicesStr.indexOf('c)') + 2, choicesStr.indexOf('d)')).trim();
        let dText = choicesStr.substring(choicesStr.indexOf('d)') + 2).trim();

        let choices = [
            { id: `c-${qIdx} -a`, text: aText },
            { id: `c - ${qIdx} -b`, text: bText },
            { id: `c - ${qIdx} -c`, text: cText },
            { id: `c - ${qIdx} -d`, text: dText }
        ];

        let correctKey = answers[qIdx].toLowerCase();
        let correctId = choices[0].id; // fallback
        if (correctKey.startsWith('b')) correctId = choices[1].id;
        if (correctKey.startsWith('c')) correctId = choices[2].id;
        if (correctKey.startsWith('d')) correctId = choices[3].id;

        parsed.push({
            id: `q - livros - poeticos - ${qIdx} `,
            disciplineId: "disc-17", // Livros Poéticos
        ...
        qIdx++;
    } else if (line.includes('Verdadeiro ou Falso:')) {
        let questionText = line.replace('Verdadeiro ou Falso:', '').replace('( )', '').trim();
        let correctKey = answers[qIdx].toLowerCase();
        let correctAns = correctKey.startsWith('v') ? 'true' : 'false';
