...
const SHIFTS = [
    ...
    { value: "hibrido", label: "Híbrido (Presencial/EAD)" },
]
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
                <label className="text-xs font-semibold text-muted-foreground block mb-1">Nome do Núcleo *</label>
                <input
            ...
            <div className="sm:col-span-2">
                ...
                <div className="flex flex-wrap gap-2">
                    {DAYS.map(d => {
                        const selectedDays = (val.dayOfWeek || "").split(",").filter(Boolean);
                        const isChecked = selectedDays.includes(d.value);
                        return (
                            <label key={d.value} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border cursor-pointer transition-colors text-xs font-medium ${isChecked ? "bg-accent/10 border-accent text-accent" : "bg-card border-border hover:border-accent/50"}`}>
                                <input
                                    type="checkbox"
                                    className="hidden"
                                    checked={isChecked}
                                    onChange={e => {
                                        ...
                                        onChange("dayOfWeek", newDays.join(","));
                                    }}
                                />
                                {d.label}
                            </label>
...
export function ClassManager() {
    ...
    async function handleDelete(id: string) {
        if (!confirm("Excluir este núcleo? Alunos vinculados perderão a associação.")) return
    ...
    function downloadClassPDF(c: ClassRoom, classStudents: StudentProfile[]) {
        ...
        const dayValues = (c.dayOfWeek || "").split(",").filter(Boolean);
        const dayNames = dayValues.map(dv => DAY_LABEL[dv] || dv).join(", ");
        doc.text(`Turno: ${SHIFT_LABEL[c.shift] || c.shift} | Dias: ${dayNames || 'N/D'}`, 14, 28)
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
                        <GraduationCap className="h-5 w-5 text-accent" /> Núcleo
                    </h2>
                    <p className="text-sm text-muted-foreground">Gerencie núcleos, dias e vagas disponíveis</p>
                </div>
                <div className="flex gap-2">
                    <button
                        ...
                        className="flex items-center gap-2 border-2 border-accent text-accent font-bold px-4 py-2 rounded-xl hover:bg-accent/10 transition-colors text-sm"
                    >
                        Sincronizar Grade
                    </button>
                    <button
            ...
            {showNew && (
                <div className="bg-accent/5 border-2 border-accent/20 rounded-2xl p-5 space-y-4">
                    <h3 className="font-semibold text-sm flex items-center gap-2"><Plus className="h-4 w-4 text-accent" /> Novo Núcleo</h3>
                    ...
                    <div className="flex gap-3">
                        <button onClick={handleAdd} disabled={saving || !form.name.trim()}
                            ...
                            {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Salvar Núcleo
                        </button>
                                        ...
                                        <div className="flex flex-wrap items-center gap-3 mt-1">
                                            {c.dayOfWeek && (
                                                <span className="flex items-center gap-1 text-xs text-muted-foreground">
                                                    ...
                                                    {(c.dayOfWeek || "").split(",").map(dv => DAY_LABEL[dv] || dv).join(", ")}
                                                </span>
                                            )}
                                            <span className="flex items-center gap-1 text-xs text-muted-foreground">
