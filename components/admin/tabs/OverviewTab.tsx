...
import { BarChart3, BookOpen, Download, FileText, GraduationCap, Users } from "lucide-react"
...
export function OverviewTab({ assessments, submissions, questions, disciplines }: Props) {
  ...
  useEffect(() => {
    if (assessments.length > 0 && (!selectedAssessmentId || !assessments.find(a => a.id === selectedAssessmentId))) {
    ...
    <div className="flex flex-col gap-6">
      ...
      {assessments.length > 0 && (
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-card border border-border/50 rounded-2xl p-6 premium-shadow">
          <div>
            <h3 className="font-bold text-foreground text-lg">Resultados por Prova</h3>
            <p className="text-sm text-muted-foreground">Selecione uma prova para visualizar as estatísticas detalhadas de desempenho.</p>
          </div>
