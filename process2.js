const fs = require('fs');
const rawText = `Unidade 1 - O Livro de Jó
...
Múltipla Escolha: Qual foi a atitude imediata de Jó após receber a notícia da morte de seus dez filhos?  a) Ele amaldiçoou o dia de seu nascimento. b) Ele buscripting vingança contra os sabeus. c) Ele prostrou-se em terra e adorou, reconhecendo a soberania de Deus. d) Ele questionou imediatamente a justiça divina com seus amigos.
...
Múltipla Escolha: O Saltério é organized em cinco livros menores. Essa estrutura é frequentemente comparada a qual outra seção da Bíblia?  a) Aos quatro Evangelhos. b) Ao Pentateuco (Torá). c) Aos Profetas Menores. d) Às Epístolas Paulinas.
...
const answers1 = ["b", "c", "b", "c", "d", "b", "c", "c", "b", "Falso", "b", "b", "c", "b", "c", "b", "c", "b", "c", "Falso", "b", "c", "b", "c", "d", "c", "b", "b", "c", "Verdadeiro", "c", "b", "c", "c", "c"];
const answers2 = ["c", "b", "b", "c", "Verdadeiro", "b", "b", "c", "d", "c", "b", "c", "c", "b", "Verdadeiro", "b", "b", "c", "b", "b", "b", "b", "b", "c", "d", "b", "b", "b", "b", "Falso", "b", "c", "b", "b", "b"];
...
for (let q of blocks) {
    q = q.trim().replace(/\n/g, ' ');
    if (!q || q.startsWith("Capítulo") || q.startsWith("Unidade") || q.startsWith("PDF")) continue;

    if (q.startsWith("Múltipla Escolha") || q.startsWith("Alternativa INCORRETA")) {
        let text = q.replace(/^(Múltipla Escolha|Alternativa INCORRETA):\s*/, '');
        let textOnly = text.split(/\s*a\)/)[0].trim();

        let choicesStr = text.substring(textOnly.length).trim();
        const aMatch = choicesStr.match(/a\)(.*?)(b\)|c\)|d\)|$)/);
        const bMatch = choicesStr.match(/b\)(.*?)(c\)|d\)|$)/);
        const cMatch = choicesStr.match(/c\)(.*?)(d\)|$)/);
        const dMatch = choicesStr.match(/d\)(.*?)$/);

        const choices = [];
        if (aMatch) choices.push({ id: `c-lp-${questionIdx}-a`, text: aMatch[1].trim() });
        if (bMatch) choices.push({ id: `c-lp-${questionIdx}-b`, text: bMatch[1].trim() });
        if (cMatch) choices.push({ id: `c-lp-${questionIdx}-c`, text: cMatch[1].trim() });
        if (dMatch) choices.push({ id: `c-lp-${questionIdx}-d`, text: dMatch[1].trim() });

        let correctAns = choices[0]?.id;
        const ansKey = answers[questionIdx].toLowerCase();
        if (ansKey.startsWith('a')) correctAns = choices[0]?.id;
        else if (ansKey.startsWith('b')) correctAns = choices[1]?.id;
        else if (ansKey.startsWith('c')) correctAns = choices[2]?.id;
        else if (ansKey.startsWith('d') && choices.length > 3) correctAns = choices[3]?.id;

        parsed.push({
            id: `q-livros-poeticos-${questionIdx}`,
        ...
        questionIdx++;
    } else if (q.startsWith("Verdadeiro ou Falso")) {
        let text = q.replace(/^Verdadeiro ou Falso:\s*/, '').replace('( )', '').trim();
        const ansKey = answers[questionIdx].toLowerCase();
        const correctAns = ansKey.startsWith('v') ? 'true' : 'false';
...
console.log('Parsed ' + parsed.length + ' questions. Writing to JSON.');

fs.writeFileSync('e:/Projetos AntiGrafity/ESCOLA DE TEOLOGIA/Escola de Teologia/IBAD/public/seed_livros_poeticos.json', JSON.stringify(parsed, null, 2), 'utf-8');
